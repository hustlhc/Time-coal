                    ...
                    "coal_infos": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                ...
                                "sulfur_pct": {"type": "number"},
                                "heat_value": {"type": "number"}
                            },
                            "required": ["forecast_prices", "current_price", "sulfur_pct", "heat_value"]
                        }
                    },
...
def run_inventory_strategy(
    ...
    base_demand, max_stock, prev_stock, extra_factor,
    weekly_accuracy,
    target_heat,target_sulfur,
    threshold_pct=0.05, weeks=4,
    month: int = 0, single_month: bool = False, **kwargs
):
    # ---------- Step1：涨跌概率 ----------
...
def compute_drop_rise_probs(coal_infos, threshold_pct=0.05, weeks=4):
    ...
    for info in coal_infos:
        ...
        # 煤种价值权重
        val = (info["heat_value"] / curr) * (1 / (info["sulfur_pct"]/100))
        values.append(val)
...
def compute_purchase_days_and_blending(week_qty, coal_infos, target_heat,target_sulfur,month: int = 0, single_month: bool = False):
    """
    ...
    for w in range(num_weeks):
        ...
        for d in range(num_days_per_week):
            ...
            prob += lpSum(x[c] * coal_infos[c]["sulfur_pct"] for c in range(num_coals)) <= wq * TARGET_SULFUR
