...
# 超参数循环设置（在这里修改即可）
# =============================
seq_len_list="96 112 120"
e_layers_list="3 4"
down_sampling_window_list="1"
d_model_list="256"
d_ff_list="256"
      ...
      for d_model in $d_model_list; do
        for d_ff in $d_ff_list; do
          for batch_size in $batch_size_list; do
            ...
            # 日志文件（包含所有关键参数）
            log_file="logs/CCI4700outnew_${seq_len}_${e_layers}_${down_sampling_layers}_${down_sampling_window}_${d_model}_ff${d_ff}_bs${batch_size}.log"
            mkdir -p logs

            # 运行脚本
            python -u run.py \
              --task_name long_term_forecast \
              --csv_path ./pre_coal/coal_CCI4700outnew_result_${seq_len}_${e_layers}_${down_sampling_layers}_${down_sampling_window}_${d_model}_ff${d_ff}_bs${batch_size}.csv \
              --is_training 1 \
              --root_path ./dataset/pre_coal/ \
              --data_path coal_new.csv \
              --model_id CCI4700outnew_TimeMixer_${seq_len}_60 \
              --model $model_name \
              --data coal \
              --features MS \
              --target_features 1 \
